...
L = 500;            % Length of signal (number of samples)
t = (0:L-1)*T;       % Time vector

% Example signal (sine wave)
f_signal_1 = 5;       % Frequency of sine wave (in Hz)
f_signal_2 = 10;       % Frequency of sine wave (in Hz)
phase_shift = pi/4;  % Phase shift (in radians)

signal = 0.7*sin(2*pi*f_signal_1*t) + 0.5*sin(2*pi*f_signal_2*t + phase_shift);
...
subplot(3,1,1)
plot(t, signal);
title('Combination of Sinusoids');
xlabel('Time (t) [sec]');
ylabel('Amplitude');
